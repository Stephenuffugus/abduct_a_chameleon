...
(async()=>{ await wait(160);pump(20);
 // TITLE Enter -> MODE_SELECT; HUNT is the 5th mode. navigate down to it (grid: index 4). menuIndex nav is linear.
 key('Enter');pump(6);
 // select HUNT via menuIndex: press ArrowDown 4 times then Enter (grid order: Practice,Survive,Heat,Daily,Hunt,...)
 for(let i=0;i<4;i++){key('ArrowDown');pump(2);} key('Enter');pump(6);   // LEVEL_SELECT (HUNT)
 key('Enter');await wait(70);pump(8);   // DIFFICULTY_SELECT
 key('Enter');await wait(30);pump(30);  // start HUNT
 let st=window.__aac.state; console.log('mode='+st.appState+'/'+st.roundState+' modeName='+st.mode, 'hunt='+JSON.stringify(st.hunt&&{bots:st.hunt.bots,caught:st.hunt.caught}));
 pump(210); st=window.__aac.state; console.log('PLAYING hunt bots='+(st.hunt&&st.hunt.bots)+' pos='+JSON.stringify(st.playerPos));
 // fly toward nearest bot repeatedly, scan periodically, try to catch
 let caughtAny=false;
 for(let seg=0; seg<400; seg++){ st=window.__aac.state; if(!st.hunt) break;
   const pp=st.playerPos, bots=st.hunt.botPos.filter(b=>!b.caught); if(!bots.length){caughtAny=true;break;}
   let best=bots[0],bd=1e18; for(const b of bots){const d=(b.x-pp.x)**2+(b.y-pp.y)**2; if(d<bd){bd=d;best=b;}}
   const dx=best.x-pp.x,dy=best.y-pp.y; const hk=Math.abs(dx)>10?(dx>0?'KeyD':'KeyA'):null, vk=Math.abs(dy)>10?(dy>0?'KeyS':'KeyW'):null;
   if(hk)down(hk); if(vk)down(vk); if(seg%6===0) key('Space'); pump(6); if(hk)up(hk); if(vk)up(vk);
   st=window.__aac.state; if(st.hunt && st.hunt.caught>0){caughtAny=true; console.log('CAUGHT '+st.hunt.caught+'/'+st.hunt.bots+' at seg '+seg); break;}
   if(st.appState==='SUMMARY') break;
 }
 st=window.__aac.state; console.log('final appState='+st.appState+' caught='+(st.hunt?st.hunt.caught:'(summary)')+' sawCaught='+caughtAny);
