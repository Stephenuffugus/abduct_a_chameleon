...
const OUT=process.argv[2]||'/tmp/hz'; fs.mkdirSync(OUT,{recursive:true});
...
// stand at a map corner and look outward + slightly up: the "prison wall" view
await p.evaluate(()=>{ window.__hz = true;
  const oldUpd = window.__aac3dPlayer; });
await p.evaluate(()=>{ document.getElementById('tView').click(); });
await new Promise(r=>setTimeout(r,600));
await p.screenshot({path:OUT+'/sky-from-above.png'});
await p.evaluate(()=>{ document.getElementById('tView').click(); });
await new Promise(r=>setTimeout(r,900));
await p.screenshot({path:OUT+'/eye.png'});
console.log('shot');
