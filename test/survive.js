...
const htmlPath='/workspaces/abduct_a_chameleon/index.html';
...
function bp(window){
  try{ const _ls={'aac.settings.v1':JSON.stringify({tutorialSeen:true,helpAutoShown:true})}; Object.defineProperty(window,'localStorage',{configurable:true,value:{getItem:k=>k in _ls?_ls[k]:null,setItem:(k,v)=>{_ls[k]=String(v);},removeItem:k=>{delete _ls[k];},clear:()=>{for(const k in _ls)delete _ls[k];},key:i=>Object.keys(_ls)[i]||null,get length(){return Object.keys(_ls).length;}}}); }catch(_){}
...
(async()=>{
 ...
 key('ArrowDown'); pump(4);        // move to SURVIVE (index 1)
 key('Enter'); pump(6);            // LEVEL_SELECT (SURVIVE)
 key('Enter'); await wait(60); pump(8);  // select first level -> DIFFICULTY_SELECT
 // difficulty: EASY is index 0 (menuIndex set to 1 by selectLevel? it sets menuIndex=1). Pick EASY:
 key('ArrowUp'); pump(4); key('Enter'); await wait(30); pump(10);
 console.log('entered:', JSON.stringify(S()));
 pump(220); // countdown -> PLAYING
 console.log('after countdown:', JSON.stringify(S()));
 // walk continuously in the open (C stays high) until caught or many seconds pass
 let caught=false, reachedSummary=false, sawChase=false, sawBeam=false;
 // home in on the nearest UFO: hold the key(s) that reduce dx/dy each tick, staying moving (C high)
 for(let seg=0; seg<400 && !reachedSummary; seg++){
   ...
   const s2=S();
   if(s2.ufoStates.includes('CHASE')) sawChase=true;
   if(s2.ufoStates.includes('BEAM')) sawBeam=true;
   if(s2.roundState==='CAUGHT') caught=true;
   if(s2.appState==='SUMMARY') reachedSummary=true;
 }
 console.log('sawChase:',sawChase,'sawBeam:',sawBeam);
 // let any cutscene finish -> SUMMARY
 pump(160);
 const fin=S();
 console.log('final:', JSON.stringify(fin), 'sawCAUGHT:', caught);
